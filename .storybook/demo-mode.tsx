...
async function mouseTo(
  ...
  if (target) {
    return new Promise((resolve) => {
      ...
      let cursorEl: HTMLDivElement | null =
        document.querySelector("#sb-demo-cursor");
      if (!cursorEl) {
...
export const demoModeLoader: Loader = async (context) => {
  const isTestRunner =
    globalThis.navigator.userAgent.match(/StorybookTestRunner/);
  ...
  if (
    ...
    context.userEvent = userEvent as unknown as ReturnType<
